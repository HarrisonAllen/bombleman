...
GBC_Graphics *GBC_Graphics_ctor(Window *window, uint8_t num_vram_banks, uint16_t *vram_sizes, uint8_t num_backgrounds) { 
    ...
    *(GBC_Graphics * *)layer_get_data(self->graphics_layer) = self;
    ...
    uint32_t total_vram_size = 0;
    for (uint8_t i = 0; i < num_vram_banks; i++) {
        total_vram_size += vram_sizes[i];
    }
    self->vram = (uint8_t*)malloc(total_vram_size);
    self->vrams[0] = self->vram;
    for (uint8_t i = 1; i < GBC_VRAM_BANK_COUNT; i++) {
        if (i < num_vram_banks) {
            self->vrams[i] = self->vrams[i-1] + vram_sizes[i-1];
        } else {
            self->vrams[i] = self->vrams[i-1];
        }
    }
...
void GBC_Graphics_destroy(GBC_Graphics *self) {
    ...
    if (self->oam != NULL)
        free(self->oam);
    if (self->bg_tilemaps != NULL)
        free(self->bg_tilemaps);
    if (self->bg_attrmaps != NULL)
        free(self->bg_attrmaps);
    if (self->bg_palette_bank != NULL)
        free(self->bg_palette_bank);
    if (self->sprite_palette_bank != NULL)
        free(self->sprite_palette_bank);
    if (self->bg_scroll_x != NULL)
        free(self->bg_scroll_x);
    if (self->bg_scroll_y != NULL)
        free(self->bg_scroll_y);
    if (self->graphics_layer != NULL)
        layer_destroy(self->graphics_layer);
    if (self != NULL)
        free(self);
}

void GBC_Graphics_release_vram(GBC_Graphics *self) {
    if (self->vram != NULL) {
        free(self->vram);
        self->vram = NULL;
    }
    for (uint8_t i = 0; i < GBC_VRAM_BANK_COUNT; i++) {
        self->vrams[i] = self->vram;
    }
}

void GBC_Graphics_reallocate_vram(GBC_Graphics *self, uint8_t num_vram_banks, uint16_t *vram_sizes) {
    uint32_t total_vram_size = 0;
    for (uint8_t i = 0; i < num_vram_banks; i++) {
        total_vram_size += vram_sizes[i];
    }
...
void GBC_Graphics_vram_move_tiles(GBC_Graphics *self, uint8_t src_vram_bank, uint8_t src_tile_offset, 
    ...
    uint8_t *src_vram_offset = self->vrams[src_vram_bank] + src_tile_offset * GBC_TILE_NUM_BYTES;
    uint8_t *dest_vram_offset = self->vrams[dest_vram_bank] + dest_tile_offset * GBC_TILE_NUM_BYTES;
...
void GBC_Graphics_load_entire_tilesheet_into_vram(GBC_Graphics *self, uint32_t tilesheet_resource, 
    ...
    uint8_t *vram_offset = self->vrams[vram_bank_number] + vram_tile_offset * GBC_TILE_NUM_BYTES;
...
void GBC_Graphics_load_from_buffer_into_vram(GBC_Graphics *self, uint8_t *tile_buffer, 
                                                uint16_t tiles_to_load, uint16_t vram_tile_offset, uint8_t vram_bank_number) {
    uint8_t *vram_offset = self->vrams[vram_bank_number] + vram_tile_offset * GBC_TILE_NUM_BYTES;
    memcpy(vram_offset, tile_buffer, tiles_to_load * GBC_TILE_NUM_BYTES);
}

uint8_t *GBC_Graphics_get_vram_bank(GBC_Graphics *self, uint8_t vram_bank_number) {
    return self->vrams[vram_bank_number];
}

uint8_t *GBC_Graphics_get_tile(GBC_Graphics *self, uint8_t vram_bank, uint8_t tile_index) {
    return GBC_Graphics_get_vram_bank(self, vram_bank) + tile_index * GBC_TILE_NUM_BYTES;
}
...
uint8_t *GBC_Graphics_get_bg_palette_pointer(GBC_Graphics *self, uint8_t palette_num) {
    return &self->bg_palette_bank[palette_num*GBC_PALETTE_NUM_BYTES];
}

uint8_t *GBC_Graphics_get_sprite_palette_pointer(GBC_Graphics *self, uint8_t palette_num) {
    return &self->sprite_palette_bank[palette_num*GBC_PALETTE_NUM_BYTES];
}
...
static void render_graphics(GBC_Graphics *self, Layer *layer, GContext *ctx) {
    ...
    uint8_t zoom_factor = ((self->zoom & GBC_ZOOM_LEVEL_MASK) >> GBC_ZOOM_LEVEL_SHIFT);
    const uint8_t *zt = &ZOOM_TABLE[4 * zoom_factor];
    ...
    for (self->line_y = 0; self->line_y < self->screen_height; self->line_y++) {
        ...
        for (sprite_id = 0; sprite_id < GBC_NUM_SPRITES; sprite_id++) {
            ...
            on_this_line = (self->line_y * zt[0] / zt[1] + self->screen_height * zt[2] / zt[3]) >= sprite_y && (self->line_y * zt[0] / zt[1] + self->screen_height * zt[2] / zt[3]) < (sprite_y + sprite_h)
        ...
        for(x = min_x; x < max_x; x++) {
            ...
            for (bg_num = 0; bg_num < self->num_backgrounds; bg_num++) {
                if ((self->lcdc & (GBC_LCDC_BG_1_ENABLE_FLAG << bg_num)) == 0) continue;

                map_x = ((x - self->screen_x_origin) * zt[0] / zt[1] + self->screen_width * zt[2] / zt[3]) + self->bg_scroll_x[bg_num];
                map_y = (self->line_y * zt[0] / zt[1] + self->screen_height * zt[2] / zt[3]) + self->bg_scroll_y[bg_num];
                ...
                tile = self->vrams[(((tile_attr & GBC_ATTR_VRAM_BANK_MASK) >> 3))] + offset; // self->vram + vram_bank_number * GBC_VRAM_BANK_NUM_BYTES + offset
            ...
            for (sprite_line_index = 0; sprite_line_index < num_sprites_on_this_line; sprite_line_index++) {
                if ((self->lcdc & GBC_LCDC_SPRITE_ENABLE_FLAG) == 0) continue;
                ...
                sprite_x = sprite[GBC_OAM_X_POS_BYTE] + (((bool)(sprite[GBC_OAM_EXTRA_BYTE] & GBC_OAM_SPRITE_UPPER_X_BIT_FLAG)) << 8) - GBC_SPRITE_OFFSET_X + self->screen_x_origin * zt[0] / zt[1];
                sprite_y = sprite[GBC_OAM_Y_POS_BYTE] + (((bool)(sprite[GBC_OAM_EXTRA_BYTE] & GBC_OAM_SPRITE_UPPER_Y_BIT_FLAG)) << 8) - GBC_SPRITE_OFFSET_Y;
                sprite_w = (1 + ((sprite[GBC_OAM_DIMS_BYTE] & GBC_OAM_SPRITE_WIDTH_MASK) >> GBC_OAM_SPRITE_WIDTH_SHIFT)) << 3; // sprite_width * tile_width
                sprite_h = (1 + ((sprite[GBC_OAM_DIMS_BYTE] & GBC_OAM_SPRITE_HEIGHT_MASK) >> GBC_OAM_SPRITE_HEIGHT_SHIFT)) << 3; // sprite_height * tile_height

                // Find the pixel on the sprite
                x_on_sprite = x * zt[0] / zt[1] + self->screen_width * zt[2] / zt[3] - sprite_x;
                if ((x * zt[0] / zt[1] + self->screen_width * zt[2] / zt[3] < sprite_x) || (x_on_sprite >= sprite_w)) continue;
                y_on_sprite = self->line_y * zt[0] / zt[1] + self->screen_height * zt[2] / zt[3] - sprite_y;
                ...
                tile = self->vrams[((sprite[GBC_OAM_ATTR_BYTE] & GBC_ATTR_VRAM_BANK_MASK) >> 3)] + offset; // self->vram + vram_bank_number * GBC_VRAM_BANK_NUM_BYTES (8192) + offset
...
void GBC_Graphics_zoom_set_level(GBC_Graphics *self, uint8_t level) {
    modify_byte(&self->zoom, GBC_ZOOM_LEVEL_MASK, level, GBC_ZOOM_LEVEL_START);
}
...
uint8_t *GBC_Graphics_oam_get_sprite_tile(GBC_Graphics *self, uint8_t sprite_num) {
    return GBC_Graphics_get_tile(
             self, 
             GBC_Graphics_attr_get_vram_bank(
               GBC_Graphics_oam_get_sprite_attrs(
                 self, sprite_num
               )
              ), 
              GBC_Graphics_oam_get_sprite_tile_pos(
                self, sprite_num
              )
           );
}
