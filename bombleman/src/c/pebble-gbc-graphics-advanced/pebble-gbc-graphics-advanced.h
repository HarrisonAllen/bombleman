...
#define GBC_VRAM_BANK_COUNT 4 ///> The max # of vram banks
/**
...
#define GBC_SPRITE_OFFSET_X 32 ///> The x offset to allow for offscreen rendering
#define GBC_SPRITE_OFFSET_Y 32 ///> The y offset to allow for offscreen rendering
...
/** Zoom Level flags */
#define GBC_ZOOM_LEVEL_MASK 0x3     ///> Mask for zoom level
#define GBC_ZOOM_LEVEL_SHIFT 0      ///> Shift for zoom level
#define GBC_ZOOM_LEVEL_START 0x01    ///> LSB of the OAM sprite width
/** Zoom levels */
#define GBC_ZOOM_LEVEL_1X 0   ///> 1x zoom level
#define GBC_ZOOM_LEVEL_2X 4   ///> 2x zoom level
#define GBC_ZOOM_LEVEL_3X 5   ///> 3x zoom level
#define GBC_ZOOM_LEVEL_4X 6   ///> 4x zoom level
/** Zoom table
 * E.g. for 2x zoom level:
 *    (x * 1 / 2 - screen_x_origin + screen_width * 1 / 4) + scroll_x + screen_x_origin * 1 / 4;
 * -> (x * ZT[4 * 4 + 0] / ZT[4 * 4 + 1] - screen_x_origin + screen_width * ZT[4 * 4 + 2] / ZT[4 * 4 + 3]) + scroll_x + screen_x_origin * ZT[4 * 4 + 2] / ZT[4 * 4 + 3]
 */
static const uint8_t ZOOM_TABLE[] = { ///> Table for each zoom level
    1, 1, 0, 1,  ///> 1x zoom level
    1, 2, 1, 4,  ///> 2x zoom level
    1, 3, 1, 3,  ///> 3x zoom level
    1, 4, 3, 8   ///> 4x zoom level
};
...
#define GBC_ZOOM_DIV(x, a, b) ((x) * (a) / (b))
...
struct _gbc_graphics {
    ...
    */
    ...
    /**
     * The Zoom byte
     *  -Bits 0-1: Zoom level (2^level, e.g. level 2 == 4x zoom), 0-3
     *  -Bits 2-7: Reserved
     */
    uint8_t zoom;
    /**
     ...
     * Now, the number of banks and sizes is set in the ctor
     */
    uint8_t *vram;
    /**
     * Takes in up to 4 vram sizes
     * No limit on size anymore (for better or worse)
     * Allocates one large vram as sum of sizes @ *vram
     * Then assigns each vram pointer like so:
     * vrams[0] = vram
     * vrams[1] = vrams[0] + vram_sizes[0]
     * vrams[2] = vrams[1] + vram_sizes[1]
     * etc.
     */
    uint8_t *vrams[4];
    /**
 ...
 * @param vram_sizes The size of each vram bank, pointer to a 4 wide uint8 array
 ...
 * 
 * @param self A pointer to the target GBC graphics object
 * @param self The number of vram banks to reallocate to
 */
...
/**
 * Loads tiles from a buffer into vram
 * 
 * @param self A pointer to the target GBC Graphics object
 * @param tiles_to_load The number of tiles to load from the tilesheet
 * @param vram_tile_offset The tile number offset on the VRAM to loading into
 ...
 * 
 * @param self A pointer to the target GBC Graphics object
 * @param level The zoom level, 0-3
 */
