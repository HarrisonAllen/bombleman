...
COLOR_INC = 255//(COLORS_PER_PALETTE-1) # So basically 16 colors, 000000 -> (0, 0, 0) = transparent, 111111 -> (17, 17, 17) = 1st, etc.
SPRITE_PALETTE = [tuple([COLOR_INC * i]*3 + [0 if i == 0 else 255]) for i in range(COLORS_PER_PALETTE)] 
...
def extract_tilesheet_tiles(tilesheet_image, sprite_width, sprite_height):
    tiles = []
    ...
    for x_sprite in range(0, dims[0], TILE_WIDTH * sprite_width):
        for y_sprite in range(0, dims[1], TILE_HEIGHT * sprite_height):
            ...
            sprite_dims = sprite.size
            for y in range(0, sprite_dims[1], TILE_HEIGHT):
                for x in range(0, sprite_dims[0], TILE_WIDTH):
                    tile = sprite.crop((x, y, x + TILE_WIDTH, y + TILE_HEIGHT))
                    tiles.append(tile)
    return tiles
...
# filename: the name of the spritesheet
    # Assumes it's already in a  #000, #111, #222, etc. color format
# sprite_width: the width (in tiles) of the sprite
# sprite_height: the height (in tiles) of the sprite
def convert_spritesheet(directory, filename, sprite_width, sprite_height):
    # Setup output directory
    print("Setting up output directory...")
    output_directory = os.path.join(directory, "output", os.path.basename(filename).split(".")[0])
    ...
    # tilesheet_palettes = extract_palettes_from_images([tilesheet_image]) # TODO: make this hardcoded?
    tilesheet_palettes = [SPRITE_PALETTE]
    tilesheet_tiles = extract_tilesheet_tiles(tilesheet_image, sprite_width, sprite_height)
...
if __name__ == "__main__":
    convert_spritesheet(sys.argv[1], sys.argv[2], int(sys.argv[3]), int(sys.argv[4]))